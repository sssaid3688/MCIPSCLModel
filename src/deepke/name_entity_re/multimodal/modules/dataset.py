...
from transformers import BertTokenizer, CLIPProcessor,CLIPTokenizer
...
class MMPNERProcessor(object):
    def __init__(self, data_path, args) -> None:
        ...
        # self.data_path['train']='/home/oyhj/IdeaProjects/DeepKE-main/example/ner/multimodal/data/twitter2015/train.txt'
        # self.data_path['train_auximgs']='/home/oyhj/IdeaProjects/DeepKE-main/example/ner/multimodal/data/twitter2015/twitter2015_train_dict.pth'
        self.tokenizer = BertTokenizer.from_pretrained(args.bert_name, do_lower_case=True)
        # self.tokenizer = CLIPTokenizer.from_pretrained(args.vit_name, do_lower_case=True)
...
class MMPNERDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        token_type_ids=[0]*self.max_seq
        labels = [self.label_mapping["[CLS]"]] + labels + [self.label_mapping["[SEP]"]] + [self.ignore_idx]*(self.max_seq-len(labels)-2)
        labels_fx = [self.label_mapping["[CLS]"]] + labels_fx + [self.label_mapping["[SEP]"]] + [self.ignore_idx]*(self.max_seq-len(labels_fx)-2)
