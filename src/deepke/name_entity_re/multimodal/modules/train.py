...
class Trainer(object):
    def __init__(self, train_data=None, dev_data=None, test_data=None, model=None, process=None, label_map=None, args=None, logger=None,  writer=None,con_lr=None,hie_lr=None,weight_decay=1e-2) -> None:
    ...
    def train(self):
        ...
        # with tqdm(total=3180, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True, initial=self.step) as pbar:
            self.pbar = pbar
            avg_loss = 0
            for epoch in range(1, self.args.num_epochs+1):
                ...
                for batch in self.train_data:
                    ...
                    attention_mask, labels,labels_fx, logits, loss = self._step(batch, mode="train")
                    ...
                    label_ids = labels_fx.to('cpu').numpy()
            ...
            self.logger.info("***********************************************************************************")
            self.logger.info("the start constrast lr is {},the start hiera lr is {},the weight_decay is {}".format(self.con_lr,self.hie_lr,self.weight_decay))
            self.logger.info("the start bert lr is {},the start vit lr is {},the k is {}".format(self.args.lr,self.args.vit_lr,self.args.k))
            self.logger.info("***********************************************************************************\n\n\n\n")
        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.dev_data:
                    ...
                    attention_mask, labels,labels_fx , logits, loss = self._step(batch, mode="dev")    # logits: batch, seq, num_labels
    ...
    def _step(self, batch, mode="train"):
        input_ids, token_type_ids, attention_mask, labels, labels_fx,images,aux_imgs= batch
        rcnn_imgs=None
    ...
    def multiModal_before_train(self):
        ...
        for name, param in self.model.named_parameters():
            if 'text' in name and 'cross' not in name and 'contrastive' not in name:
                params['params'].append(param)
        parameters.append(params)

         # vit lr
        params = {'lr':self.args.vit_lr, 'weight_decay':1e-2}
        params['params'] = []
        for name, param in self.model.named_parameters():
            if 'vision' in name and 'cross' not in name and 'contrastive' not in name:
                params['params'].append(param)
        parameters.append(params)
        ...
        # params['params'] = []
        # for name, param in self.model.named_parameters():
        #     if 'Biencoder' in name:
        #         params['params'].append(param)
        # parameters.append(params)
        #

        params = {'lr': self.hie_lr, 'weight_decay': self.weight_decay}
        params['params'] = []
        for name, param in self.model.named_parameters():
            if 'cross' in name:
                params['params'].append(param)
        ...
        params = {'lr': self.con_lr, 'weight_decay': self.weight_decay}
        params['params'] = []
        for name, param in self.model.named_parameters():
